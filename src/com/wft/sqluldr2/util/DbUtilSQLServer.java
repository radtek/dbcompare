...
public class DbUtilSQLServer {
	...
	private void loadConfig() {
		try {
			...
			DBUrl = ConstantsTools.CONFIGER.getFromHost();
			DBName = ConstantsTools.CONFIGER.getFromName();
			DBUser = ConstantsTools.CONFIGER.getFromUser();
			DBPassword = ConstantsTools.CONFIGER.getFromPassword();
	...
	public synchronized Connection getConnection() throws SQLException {
		...
		// 当DB配置变更时重新获取
		if (!ConstantsTools.CONFIGER.getFromHost().equals(DBUrl)
				|| !ConstantsTools.CONFIGER.getFromName().equals(DBName)
				|| !ConstantsTools.CONFIGER.getFromUser().equals(DBUser)
				|| !ConstantsTools.CONFIGER.getFromPassword().equals(DBPassword)) {
			loadConfig();
		...
		if (connection == null || connection.isValid(10) == false) {
			// "jdbc:sqlserver://20.1.1.194:1433;DatabaseName=AIS20151221115438;"
			connection = DriverManager.getConnection("jdbc:sqlserver://" + DBUrl + ";DatabaseName=" + DBName, user, password);
			// 把事务提交方式改为手工提交
			connection.setAutoCommit(false);
		}
	 ...
	 * @throws SQLException
	 */
	public synchronized Connection testConnection(String DBUrl, String DBName, String DBUser, String DBPassword) throws SQLException {
		loadConfig();
		// "jdbc:sqlserver://20.1.1.194:1433;DatabaseName=AIS20151221115438;"
		connection = DriverManager.getConnection("jdbc:sqlserver://" + DBUrl + ";DatabaseName=" + DBName, DBUser, DBPassword);
		// 把事务提交方式改为手工提交
		connection.setAutoCommit(false);
