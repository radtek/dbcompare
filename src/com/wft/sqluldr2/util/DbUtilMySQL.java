...
public class DbUtilMySQL {
    ...
    private void loadConfig() {
        try {
            ...
            DBUrl = ConstantsTools.CONFIGER.getToHost();
            DBName = ConstantsTools.CONFIGER.getToName();
            DBUser = ConstantsTools.CONFIGER.getToUser();
            DBPassword = ConstantsTools.CONFIGER.getToPassword();
    ...
    public synchronized Connection getConnection() throws SQLException {
        ...
        // 当DB配置变更时重新获取
        if (!ConstantsTools.CONFIGER.getToHost().equals(DBUrl) || !ConstantsTools.CONFIGER.getToName().equals(DBName)
                || !ConstantsTools.CONFIGER.getToUser().equals(DBUser)
                || !ConstantsTools.CONFIGER.getToPassword().equals(DBPassword)) {
            loadConfig();
            // "jdbc:mysql://localhost/pxp2p_branch"
            connection = DriverManager.getConnection("jdbc:mysql://" + DBUrl + "/" + DBName + "?useUnicode=true&characterEncoding=utf8", user, password);
            // 把事务提交方式改为手工提交
